...
import carb
from pxr import Usd, Sdf, Gf # Gf を追加して Gf.Vec3d などを使えるようにする

# グローバル変数: リセットが必要なプロパティの元の値を保存するための辞書
# キー: "prim_path.attribute_name" (例: "/World/Workpiece.xformOp:translation")
# 値: その属性の元の値 (Gf.Vec3dなど)
_original_prim_properties_state = {}

# グローバル変数: タイムラインイベントの購読オブジェクト
...
class SimulationResetExtension(omni.ext.IExt):
    ...
    def on_startup(self, ext_id):
        ...
        # タイムラインイベントストリームが有効になっていることを確認
        self._timeline.set_timeline_event_stream_enabled(True)

        # タイムラインイベントを購読
        # _on_timeline_event_callback は、イベントが発生したときに呼び出されるメソッド
        # create_subscription_to_pop は、イベントが処理された後にストリームから削除される
